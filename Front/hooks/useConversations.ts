...
export function useConversations() {
  ...
  const updateChatById = useCallback((chatId: string, updater: (chat: Chat) => Chat) => {
    setChats(prevChats => prevChats.map(chat => (chat.id === chatId ? updater(chat) : chat)))
  }, [])
  ...
  const removeLoadingMessage = useCallback((chatId: string) => {
    updateChatById(chatId, chat => ({
      ...chat,
      messages: chat.messages.filter(msg => msg.content !== "..."),
    }))
  }, [updateChatById])

  const loadChatFromBackend = useCallback(async (chatHash: string) => {
    try {
      console.log('🔥 Loading chat from backend:', chatHash)
      const response = await fetch(`http://localhost:5000/getConversation/${chatHash}`)
  ...
  const loadUserConversations = useCallback(async (uid: number) => {
    try {
      ...
      const response = await fetch(`http://localhost:5000/getUserConversations/${uid}`)
      const data = await response.json()
      ...
      const chatsPromises = data.conversations.map(async (conv: any) => {
        const messagesResponse = await fetch(`http://localhost:5000/getConversation/${conv.conversation_id}`)
        ...
        if (messagesData.success && messagesData.messages) {
          ...
          const updatedTime = conv.ended_at && conv.ended_at !== null
      ...
      const loadedChats = (await Promise.all(chatsPromises)).filter(Boolean) as Chat[]
